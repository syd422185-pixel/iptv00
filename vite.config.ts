    ...
    configureServer(server: ViteDevServer) {
      server.middlewares.use("/__manus__/logs", (req, res, next) => {
        ...
        if (reqBody && typeof reqBody === "object") {
          try { handlePayload(reqBody); } catch (e) { res.writeHead(400); res.end(String(e)); }
          return;
        }
        let body = "";
        req.on("data", (chunk) => { body += chunk.toString(); });
        req.on("end", () => {
          try { const payload = JSON.parse(body); handlePayload(payload); } catch (e) { res.writeHead(400); res.end(String(e)); }
        });
      });
    },
  };
